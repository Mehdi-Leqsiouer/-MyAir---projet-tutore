<?php 
	...
	if ($dbcpolluscope = pg_Connect("host=localhost port=5432 dbname=polluscope user=postgres password=12345678 "))
	{
	    ...
	    $Array=array();
		if((isset($_GET['NodeID'])and $query1 = pg_exec($dbcpolluscope,"select *,ST_AsGeoJSON(geom) as geojson from plat_airparif_data where node_id IN ('$ids') and \"timestamp\" BETWEEN '$start1' and '$end1' order by id  "))
			or $query1 = pg_exec($dbcpolluscope,"select *,ST_AsGeoJSON(geom) as geojson from plat_airparif_data where \"timestamp\" BETWEEN '$start1' and '$end1' order by id  ")
		)
		
		{
		 ...
		 'formaldehyde'=>$row['formaldehyde'],'no2'=>$row['no2'],'GeoJSON'=>$row['geojson']);
